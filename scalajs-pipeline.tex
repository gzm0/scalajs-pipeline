...
\sK{class} MultiAlerter \{
  ...
  \visible<+>{\sK{def} $js$exported$meth$multiAlert(n: Int): Any = multiAlert(n)}
\}
...
\}
    ...
    \begin{block}{\alt<5>{}{Pseudo }JavaScript Code}
      \vspace{-\baselineskip}
      \begin{semiverbatim} \small
\sK{var} norm = \sK{function}(a2, b) \{
  \only<-2>{\sK{return} }\{
    \sK{var} a2 = a * a;
    \only<-3>{\sK{var} b2 = }\{
      \sK{var} b0 = parseInt(b, 10);
...
\begin{frame}
  ...
  \begin{columns}[t]
    ...
    \begin{block}{Linker}
      ...
      \item Instance tests
      \item Longs
      \item Class emission order
      \item JS Ctors \& Inheritance
